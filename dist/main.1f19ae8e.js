...
var CONTRACT_NAME = 'mitchelltran.testnet' || 'mitchelltran.testnet';
  ...
  switch (env) {
    ...
    case 'local':
      return {
        ...
        keyPath: "".concat("/home/manonymous", "/.near/validator_key.json"),
...
function _connect() {
  _connect = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee(nearConfig) {
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 2:
            ...
            window.walletConnection = new nearAPI.WalletConnection(window.near); // Initializing our contract APIs by contract name and configuration.

            _context.next = 6;
            return new nearAPI.Contract(window.walletConnection.account(), nearConfig.contractName, {
...
function updateUI() {
  if (!window.walletConnection.getAccountId()) {
    Array.from(document.querySelectorAll('.sign-in')).map(function (it) {
      return it.style = 'display: block;';
    });
  } else {
    Array.from(document.querySelectorAll('.after-sign-in')).map(function (it) {
      return it.style = 'display: block;';
    });
    contract.get_num().then(function (count) {
...
document.querySelector('.sign-out .btn').addEventListener('click', function () {
  walletConnection.signOut(); // TODO: Move redirect to .signOut() ^^^

  window.location.replace(window.location.origin + window.location.pathname);
});
window.nearInitPromise = connect(nearConfig).then(updateUI).catch(console.error);
